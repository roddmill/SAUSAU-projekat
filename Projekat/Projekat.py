...
# Uklanjanje vodećih i pratećih razmaka u imenima kolona
...
for kolona in broj_kolone:
    if kolona in podaci.columns:
        # Označi one vrednosti koje NISU celobrojne (tj. % 1 nije nula)
...
# Uklanjanje ekstremnih vrednosti pomoću z-score-a
...
podaci[numericke_kolone] = scaler.fit_transform(podaci[numericke_kolone])

pca = PCA(n_components=0.95)
podaci_scaled = scaler.fit_transform(podaci[numericke_kolone])
podaci_pca = pca.fit_transform(podaci_scaled)
pca_df = pd.DataFrame(podaci_pca, columns=[f'PC{i+1}' for i in range(pca.n_components_)])
...
# Decision tree regresija - 0.0249
...
# Random forrest regresija - 0.0331
...
plt.barh(top_features['Feature'][::-1], top_features['Importance'][::-1], color='dodgerblue')
plt.xlabel('Feature Importance')
plt.title(f'Top {top_n} najvažnijih atributa - Random Forest')
plt.tight_layout()
plt.show()
